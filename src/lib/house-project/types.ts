...
export interface OpeningInstance {
  ...
  variantId?: string;
  /**
   * Лента остекления: общий идентификатор у окон соседних модулей, слитых в
   * одно остекление через стык. Проём всё равно принадлежит своему модулю —
   * изделие отливается с проёмом, и окна «на два модуля» не существует. Общая
   * метка нужна, чтобы редактор и просмотр рисовали их одним элементом, а не
   * двумя (см. `opening-band.ts`).
   */
  bandId?: string;
  note?: string;
}
