...
const DEFAULTS = {
  ...
  resolution: "1k",
  quality: "low",
  // Оба лимита сняты по прямому решению владельца (10.08.2026): ноль означает
  // «без ограничения». До этого было по одной генерации на посетителя и 50 в
  // сутки на всех.
  //
  // Что это значит по деньгам. Каждая генерация оплачивается с кошелька
  // OpenRouter, настоящая цена приходит полем usage.cost с каждым ответом и
  // пишется в лог. Без дневного потолка расход ничем не ограничен сверху —
  // единственный тормоз остаётся ручной: AI_LAYOUT_KILL_SWITCH=1 гасит
  // функцию мгновенно. Вернуть потолок можно, не трогая код:
  // AI_LAYOUT_DAILY_LIMIT=50 и AI_LAYOUT_FREE_PER_VISITOR=1 в ecocub.env.
  freePerVisitor: 0,
  dailyLimit: 0,
