...
-- Права публичных ролей задаются с нуля, а не добавлением к тому, что есть.
--
-- Supabase раздаёт новым таблицам в схеме public набор прав по умолчанию, и
-- среди них TRUNCATE. Он опаснее, чем кажется: защита на уровне строк на него
-- не распространяется — это операция над таблицей целиком, способная стереть
-- все карточки разом. Через REST её не вызвать, но и оставлять такое право
-- публичной роли незачем.
--
-- Поэтому сначала снимаем всё, потом выдаём единственное нужное — SELECT.
-- INSERT/UPDATE/DELETE не выдаются никому: запись идёт только ключом
-- service_role, который права и политики обходит.
